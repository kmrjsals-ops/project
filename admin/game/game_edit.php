...
// 2. 이미지 가져오기
$img_sql = "SELECT * FROM game_images WHERE game_no = $game_no";
        ...
        <form action="game_edit_ok.php" method="post" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <input type="text" name="game_title"
                       class="form-control"
                       value="<?= htmlspecialchars($game['game_title']) ?>"
                       required>
            </div>
            ...
            <div class="mb-3">
                ...
                <input type="text" name="game_summary"
                       class="form-control"
                       value="<?= htmlspecialchars($game['game_summary']) ?>"
                       required>
            </div>
            ...
            <div class="mb-3">
                ...
                <textarea name="game_detail"
                          class="form-control"
                          rows="6"
                          required><?= htmlspecialchars($game['game_detail']) ?></textarea>
            </div>

            <!-- 게임 URL -->
            <div class="mb-3">
                <label class="form-label">게임 URL</label>
                <input type="url" name="game_url"
                       class="form-control"
                       value="<?= htmlspecialchars($game['game_url']) ?>"
                       required>
            </div>
                ...
                <?php if (count($gallery) > 0) : ?>
                    <div class="d-flex flex-wrap gap-2 mb-2">
                        <?php foreach ($gallery as $g) : ?>
                            <img src="/webzen/uploads/games/<?= htmlspecialchars($g) ?>"
                                 width="180"
